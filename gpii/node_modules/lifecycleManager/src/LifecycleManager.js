...
(function () {
    ...
    fluid.defaults("gpii.lifecycleManager", {
        ...
        retryOptions: { // Options governing how often to recheck whether settings are set (and in future, to check for running processes)
            ...
            numRetries: 12, // Make 10 attempts over a period of 10 seconds to discover whether settings are set
            retryInterval: 1000
        },
        components: {
            variableResolver: {
        ...
        invokers: {
            ...
            start: {
                ...
                args: ["{that}", "{requestProxy}", "{arguments}.0", "{arguments}.1", "{arguments}.2"]
            ...
            settingsResolver: {
                funcName: "gpii.lifecycleManager.settingsResolver",
    ...
    gpii.combineFetchers = function () {
        // this slightly akward start of the function due to:
        // http://stackoverflow.com/questions/6396046/unlimited-arguments-in-a-javascript-function
        var args = [];
        for (var i = 0; i < arguments.length; ++i) {
            args[i] = arguments[i];
        }
        // use first matched fetcher
        return function (parsed) {
            for (var i = 0; i < args.length; ++i) {
                if (typeof args[i] !== "function") {
                    fluid.log("WARNING: fetcher is not a function so ignoring: " + args[i]);
                    continue;
                }
                var fetched = args[i](parsed);
   ...
   //   http://wiki.gpii.net/index.php/Transformer_Payload_Examples
        ...
        var settingsPackage = fluid.transform(settingsHandlers, function (handlerSpec) {
            ...
            return {
                setSettings: function () {
                    var togo = gpii.settingsHandlers.invokeRetryingHandler(resolvedName, settingsHandlerPayload, that.options.retryOptions);
                    return togo;
                },
    ...
    // @toSnapshot (Object) is the structure which will be modified to form the snapshot record (aka. originalSettings) and stashed in sessionState
    // @solutionRecord (Object) is not modified and consists of the original transformer return (doesn't really appear necessary)
    // @isUpdate (boolean) indicates whether we're doing an update - in which case the originalSettings should not be written based on snapshot
    // Returns the same promise yielded by executeActions - the stateful construction of the session state is tacked onto this promise
    // as a side-effect
     ...
     */
    gpii.lifecycleManager.update = function (that, requestProxy, options, solutions, callback) {
        var userToken = options.userToken;
        var sessionState = that.activeSessions[userToken];
        if (!sessionState) {
        ...
        fluid.each(solutions, function (solution, solutionId) {
            ...
            if ($.inArray("setSettings", solution.lifecycleManager.start) !== -1) {
                actions = [ "setSettings" ];
            }
    ...
    gpii.lifecycleManager.start = function (that, requestProxy, options, solutions, callback) {
        var userToken = options.userToken;
        //log out any user that is already logged in:
        if (that.activeSessions.length !== 0) {
            // TODO: develop async architecture to prevent rat's nest of callbacks
            var activeTokens = that.getActiveSessionTokens();
            that.stop({userToken: activeTokens[0]}, fluid.identity);
        }
        var sessionState = $.extend(true, {
            actionResults: {}
        }, options);

        // build settings resolvers:
        var settingsResolver = {};
        fluid.each(solutions, function (content, id) {
            if (!content.settingsHandlers || !content.settingsHandlers[0]) {
                //ignore entries without settings handlers
                return;
            }
            fluid.each(content.settingsHandlers[0].settings, function (val, name) {
                settingsResolver["settings." + id + "." + name] = val;
            });
        });
        ...
        sessionState.localFetcher = gpii.combineFetchers(
            ...
            gpii.resolversToFetcher(settingsResolver),
        ...
        var tasks = [];
		fluid.log("mega chusca",JSON.stringify(solutions, null, 2));
        fluid.each(solutions, function (solution, solutionId) {
