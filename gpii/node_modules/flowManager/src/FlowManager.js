...
fluid.defaults("gpii.flowManager.local", {
    ...
    listeners: {
        ...
        "onCreate.loadDefaultSettings": {
            funcName: "gpii.flowManager.loadDefaultSettings",
            args: ["{that}", "{defaultSettingsLoader}", "{that}.events.defaultSettingsLoaded"]
        },
        // Convert the default settings to lifecycle instruction snapshot only once on system start.
        "defaultSettingsLoaded.convertDefaultSettingsToSnapshot": {
...
gpii.flowManager.loadDefaultSettings = function (that, defaultSettingsLoader, defaultSettingsLoadedEvent) {
    var promise = defaultSettingsLoader.prepareSettingsFile();

    promise.then(function (settingsFilePath) {
        fluid.log("## on loadDefaultSettings - settingsFilePath: ", settingsFilePath);
        that.options.defaultSettings = defaultSettingsLoader.get(settingsFilePath);
        defaultSettingsLoadedEvent.fire();
    });
};
...
gpii.flowManager.convertDefaultSettingsToSnapshot = function (that, defaultSettings, defaultSnapshotCreatedEvent) {
    if (defaultSettings) {
        ...
        matchMakingPromise.then(function (payload) {
            var activeContextName = payload.activeContextName || "gpii-default";
            // convert the matchmaker output to lifecycle instructions
            that.options.defaultLifecycleInstructions = gpii.transformer.configurationToSettings(payload.matchMakerOutput.inferredConfiguration[activeContextName], payload.solutionsRegistryEntries);
...
gpii.flowManager.local.resetAtStart = function (lifecycleManager, resetAtStart, defaultSnapshot, resetSuccessEvent, resetErrorEvent) {
    fluid.log("## at resetAtStart: ", defaultSnapshot);
