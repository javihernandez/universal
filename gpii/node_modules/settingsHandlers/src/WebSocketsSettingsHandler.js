...
(function () {
    ...
    gpii.settingsHandlers.WebSocketsSettingsHandler.getSettings = function (payload, id, that) {
    ...
    gpii.settingsHandlers.WebSocketsSettingsHandler.setSettings = function (payload, id, that) {
        var result = fluid.copy(payload);
        var newSettingsResponse = [];
        ...
        if (Object.keys(gpii.settingsHandlers.WebSocketsSettingsHandler.settings[id]).length === 0) {
    ...
    gpii.settingsHandlers.WebSocketsSettingsHandler.addBrowser = function (id, socket) {
        ...
        fluid.set(gpii.settingsHandlers.WebSocketsSettingsHandler.clients, [id, socketId], socket);
    };
    ...
    gpii.settingsHandlers.WebSocketsSettingsHandler.removeBrowser = function (socket) {
        for (var id in gpii.settingsHandlers.WebSocketsSettingsHandler.clients) {
            if (socket.id in gpii.settingsHandlers.WebSocketsSettingsHandler.clients[id]) {
                ...
                // TODO: Remove this since it's only for debugging purposes
                //
                //for (var appId in gpii.settingsHandlers.WebSocketsSettingsHandler.clients) {
                //    console.log("### appId: " + appId);
                //    for (var client in gpii.settingsHandlers.WebSocketsSettingsHandler.clients[appId]) {
