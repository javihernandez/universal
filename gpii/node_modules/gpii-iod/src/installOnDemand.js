...
fluid.defaults("gpii.iod", {
    ...
    events: {
        ...
        // We need such events to inform the user that IoD is working
        onInstallOnDemandStartedWorking: null,
        onInstallOnDemandStoppedWorking: null
    },
    listeners: {
        ...
        // In SmartWork we don't need to uninstall stuff for now.
    ...
    invokers: {
        ...
        readInstallationsFile: {
            funcName: "gpii.iod.readInstallationsFile",
            args: "{gpii.journal}.settingsDir.gpiiSettingsDir"
        },
        updateInstallationsFile: {
            funcName: "gpii.iod.updateInstallationsFile",
            args: ["{that}", "{gpii.journal}.settingsDir.gpiiSettingsDir", "{arguments}.0"]
        },
        unrequirePackage: {
...
/**
 * Reads information about installed packages.
 *
 * @param {String} directory The directory containing the stored installation info.
 * @return {Object} The installations data.
 */
gpii.iod.readInstallationsFile = function (directory) {
    var filename = path.join(directory, "iod-installations.json");
    return fs.existsSync(filename) ? JSON.parse(fs.readFileSync(filename)) : {};
};

/**
 * Writes information about installed packages so they can be updated at a later time.
 *
 * @param {Component} that The gpii.iod instance.
 * @param {String} directory The directory containing the stored installation info.
 * @param {Object} packageData The packageData of the recently installed package.
 */
gpii.iod.updateInstallationsFile = function (that, directory, packageData) {
    var filename = path.join(directory, "iod-installations.json");

    var installations = that.readInstallationsFile();
    installations[packageData.name] = packageData._original.version;
    fs.writeFileSync(filename, JSON.stringify(installations));
};
...
gpii.iod.requirePackage = function (that, packageRequest) {
    ...
    that.packages.getPackageData(packageRequest).then(function (packageData) {
        ...
        // SW-39: First we check whether IoD reports it as installed, if so, we check
        // the version installed and mark it as non installed if there's an older version.
        var checkInstalled = that.packages.checkInstalled(packageData);
        if (checkInstalled) {
            var installedVersion = that.readInstallationsFile()[packageData.name];
            isInstalled = semver.gt(packageData.version, installedVersion) ? false : true;
        };
        ...
        if (!isInstalled) {
            that.initialiseInstallation(packageData).then(function (installation) {
                ...
                if (installation.installed) {
                    ...
                    // We fire this event to indicate that IoD is installing software
                    that.events.onInstallOnDemandStartedWorking.fire();
			...
			// although auto-removal is disabled for now in Smartwork.
                        that.writeInstallation(installation);
                        // SW-39 - TODO: The auto-update feature must be properly integrated into
                        // the original IoD workflow. Right now it works but in the future we will
                        // need to support the uninstall functionality per users request.
                        that.updateInstallationsFile(packageData);
