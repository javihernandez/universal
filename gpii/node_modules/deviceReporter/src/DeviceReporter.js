...
gpii.deviceReporter.fireResponse = function (requestProxy, installedSolutions, platformReporter) {
...
gpii.deviceReporter["static"].get = function (requestProxy, deviceReporter) {
    deviceReporter.installedSolutionsDataSource.get(null, function onSuccess(solutions) {
...
fluid.defaults("gpii.deviceReporter.live", {
    ...
    //        type: "{flowManager}.solutionsRegistryDataSource"
    //    }
    //},
    invokers: {
        get: {
...
gpii.deviceReporter.live.get = function (requestProxy, deviceReporter, solutionsRegistryDataSource) {
    ...
    solutionsRegistryDataSource.get(null, function onSuccess(entries) {
        fluid.each(entries, function (entry, entryId) {
            if (!installedSolutions.some(function (s) { return s.id === entryId; })) {
                var foundEntryId = fluid.find(entry.contexts.isInstalled, function (installedSolutionsReporter) {
                    if (fluid.invokeGradedFunction(installedSolutionsReporter.type, installedSolutionsReporter)) {
                        return entryId;
                    }
                }, null);
        ...
        deviceReporter.fireResponse(requestProxy, installedSolutions);
    });
};
